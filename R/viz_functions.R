...
simple_pnts <- function(pnts, title = NULL, add = FALSE, ...){
  plot(pnts, add=add, col="red", cex=.5, pch =20, main = title)
}

simple_poly <- function(poly, title = NULL, add = FALSE, ...){
...
heatmap <- function(pnts, title = NULL, add = FALSE, ...){
  ppp <- as.ppp(pnts)
  D <- density(ppp)
  D <- as(D, "RasterLayer")
...
gg_poly <- function(poly, basemap = basemap, title = NULL, ...) {
  poly <- spTransform(poly, WGS84)
